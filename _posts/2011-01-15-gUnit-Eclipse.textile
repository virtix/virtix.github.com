...
# "Download ANTLR":http://www.antlr.org/download.html and "Eclipse":http://www.eclipse.org/downloads/
...
I have a simple grammar, @SQLToDNF.g@, which I'll share as it matures. My tests are rudimentary, but a _baby step_ in the right direction. Note the tes "test suites" below correlate to the elements within the grammar and each individual test is a String that is processed by the generated parser. Here I use only @OK@ and @FAIL@ assertions, though there are a few others.
...
* How can I best create and run test suites with hundreds of tests? Do I just generate jUnit tests from the gUnit grammar, or is there a better way?
* The output is minimal. An optional stacktrace would be nice. It would also be nice to be able to label tests and, like jUnit, offer some hints as to why a test failed. Perhaps something like:
...
//simple rule test
select_statement [My Custom label would go here] : 
  "SELECT * FROM foo" OK [Comment: Basic statement should pass no matter what!]
;
